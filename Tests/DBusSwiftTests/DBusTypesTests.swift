...
struct DBusTypesTests {
    ...
    func testDBusTypeFromString() throws {
        // Test basic types
        #expect(DBusType(stringValue: "y") == .byte)
        #expect(DBusType(stringValue: "b") == .boolean)
        #expect(DBusType(stringValue: "n") == .int16)
        #expect(DBusType(stringValue: "q") == .uint16)
        #expect(DBusType(stringValue: "i") == .int32)
        #expect(DBusType(stringValue: "u") == .uint32)
        #expect(DBusType(stringValue: "x") == .int64)
        #expect(DBusType(stringValue: "t") == .uint64)
        #expect(DBusType(stringValue: "d") == .double)
        #expect(DBusType(stringValue: "s") == .string)
        #expect(DBusType(stringValue: "o") == .objectPath)
        #expect(DBusType(stringValue: "g") == .signature)
        
        // Test container types
        #expect(DBusType(stringValue: "a") == .array)
        #expect(DBusType(stringValue: "v") == .variant)
        #expect(DBusType(stringValue: "r") == .struct)
        #expect(DBusType(stringValue: "e") == .dictEntry)
