...
public struct DBusMessage: Sendable {
    ...
    public var replyTo: UInt32? {
        guard case .uint32(let replyTo) = headerFields.first(where: { $0.code == .replySerial })?.variant.value else {
            return nil
        }
        return replyTo
    }
    ...
    internal init(byteOrder: Endianness, messageType: MessageType, flags: Flags, protocolVersion: UInt8, serial: UInt32, headerFields: [HeaderField], body: [DBusValue]) {
    ...
    init(from buffer: inout ByteBuffer) throws {
        ...
        self.body = try Self.parseArguments(headerFields: headerFields, body: &body, byteOrder: byteOrder)
    }
}
